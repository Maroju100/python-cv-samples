...
def avg_circles(circles, b):
    ...
    for i in range(b):
        ...
        max_y = circles[0][i][1]
        max_r = circles[0][i][2]
        cv2.circle(img, (max_x, max_y), max_r, (0, 0, 255), 3, cv2.LINE_AA)  # draw circle
        cv2.circle(img, (max_x, max_y), 2, (0, 255, 0), 3, cv2.LINE_AA)  # draw center of circle
    ...
    return p_max_x, p_max_y, p_max_r    
...
def calibrate_gauge(gauge_number, file_type):
    ...
    circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1.1, 50)
    #circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1.2, 100, np.array([]), 100, 50, int(height*0.1), int(width*0.48))
    #circles = cv2.HoughCircles(gray, cv2.HOUGH_GRADIENT, 1, height*0.1, np.array([]), 100, 50, int(height*0.1), int(height*0.48))
...
def get_current_value(img, min_angle, max_angle, min_value, max_value, x, y, r, gauge_number, file_type):
    ...
    cv2.imwrite('gauge-%s-thresholding.%s' % (gauge_number, file_type), dst2)
    print "Radius of the Dial = %s" %r
    # find lines
    minLineLength = r*0.1
    #print "minlength: %s " %minLineLength
    maxLineGap = 0
    lines = cv2.HoughLinesP(image=dst2, rho=4, theta=np.pi / 180, threshold=100,minLineLength=minLineLength, maxLineGap=0)  # rho is set to 3 to detect more lines, easier to get more then filter them out later
    copyimg = copy.deepcopy(img)
    #for testing purposes, show all found lines
    for i in range(0, len(lines)):
      for x1, y1, x2, y2 in lines[i]:
         cv2.line(copyimg, (x1, y1), (x2, y2), (0, 255, 0), 2)
         cv2.imwrite('gauge-%s-show-alllines.%s' %(gauge_number, file_type), copyimg)
    ...
    for i in range(0, len(lines)):
		...
		#print "line_length %s" %line_length
                # add to final list
                #final_line_list.append([x1, y1, x2, y2])
    final_line_list.append([temp_x1, temp_y1, temp_x2, temp_y2])
    print "Dial length = %s" %value
    #testing only, show all lines after filtering
    #copyimg2 = copy.deepcopy(img)
    ...
    #for i in range(0,len(final_line_list)):
         ...
         #cv2.line(copyimg2, (x1, y1), (x2, y2), (0, 255, 0), 2)
    	 #cv2.imwrite('gauge-%s-linesfiltered-2.%s' % (gauge_number, file_type), copyimg2)
    ...
    cv2.imwrite('gauge-%s-lineoverlayed.%s' % (gauge_number, file_type), img)
...
def main():
    ...
    client.publish("topic/state", val)#publish
